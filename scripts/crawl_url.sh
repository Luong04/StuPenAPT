...
mkdir -p "$OUTPUT_DIR"/{katana,gau,ffuf,dirlist,summary}
mkdir -p "$TEMP_DIR"
...
STATIC_FILTER='\.(png|jpg|jpeg|gif|bmp|webp|svg|ico|cur|tiff?|psd|css|scss|sass|less|woff|woff2|ttf|eot|otf|mp4|mp3|avi|mov|wmv|flv|webm|mkv|wav|ogg|m4a|m4v|3gp|f4v|aac|flac|wma|opus|swf)(\?|$|#)'
JS_FILTER='^[^?]*\.(js|map)(\?|$|#)'
...
# ═══════════════════════════════════════════════════════════════════════════
echo -e "${BLUE}[1/6] Detecting web framework...${NC}"
...
# ═══════════════════════════════════════════════════════════════════════════
echo -e "${BLUE}[3/6] GAU (archived URLs)...${NC}"
...
# ═══════════════════════════════════════════════════════════════════════════
echo -e "${BLUE}[4/6] ffuf smart fuzzing (ffuf_crawl.sh)...${NC}"
...
# ═══════════════════════════════════════════════════════════════════════════
# STEP 5: DIRECTORY LISTING CRAWL
#   - Extract directory paths from all discovered URLs
#   - Detect Apache/Nginx/Python directory listings
#   - Recursively crawl listed files & subdirectories (max depth 3)
#   - Output: dirlist/endpoints.txt, dirlist/endpoints_detail.txt
# ═══════════════════════════════════════════════════════════════════════════
echo -e "${BLUE}[5/6] Directory listing detection...${NC}"

# Curl args for directory checks
CURL_DIR_ARGS=(-s -m 10 -H "User-Agent: $USER_AGENT" -L)
[ -n "$COOKIE_STRING" ] && CURL_DIR_ARGS+=(-H "Cookie: $COOKIE_STRING")
[ -n "$AUTH_HEADER" ]   && CURL_DIR_ARGS+=(-H "Authorization: $AUTH_HEADER")
[ -n "$PROXY" ]         && CURL_DIR_ARGS+=(--proxy "$PROXY")

TARGET_SCHEME=$(echo "$TARGET" | grep -oE '^https?')
TARGET_DOMAIN=$(echo "$TARGET" | awk -F/ '{print $3}')
TARGET_ORIGIN="${TARGET_SCHEME}://${TARGET_DOMAIN}"

# Directory listing detection pattern
DIRLIST_PATTERN='<title>\s*(Index of|Directory listing for)|<h1>\s*Index of'

# ── Collect candidate directories from all tool results ──────────────────
{
    ...
    sed 's|/[^/]*$|/|' | \
    sort -u | grep -iF "$TARGET_DOMAIN" | \
    grep -E '^https?://.+/.+' \
    > "$TEMP_DIR/dir_candidates.txt" 2>/dev/null

# Also add URLs that end with / (they ARE directories)
{
    cat "$OUTPUT_DIR/katana/endpoints.txt" \
        "$OUTPUT_DIR/gau/endpoints.txt" \
        "$OUTPUT_DIR/ffuf/endpoints.txt" 2>/dev/null
} | sed 's/[?#].*//' | \
    grep -E '/$' | grep -iF "$TARGET_DOMAIN" \
    >> "$TEMP_DIR/dir_candidates.txt" 2>/dev/null

sort -u -o "$TEMP_DIR/dir_candidates.txt" "$TEMP_DIR/dir_candidates.txt"

DIR_COUNT=$(wc -l < "$TEMP_DIR/dir_candidates.txt" 2>/dev/null || echo 0)
echo -e "${CYAN}    → ${DIR_COUNT} candidate directories${NC}"

# ── Recursive directory listing crawler ──────────────────────────────────
> "$TEMP_DIR/dirlist_urls.txt"
> "$TEMP_DIR/dirlist_checked.txt"
DIRLIST_FOUND=0
...
crawl_dirlist() {
    ...
    # Skip if already checked or too deep
    grep -qxF "$URL" "$TEMP_DIR/dirlist_checked.txt" 2>/dev/null && return
    echo "$URL" >> "$TEMP_DIR/dirlist_checked.txt"
    (( DEPTH > MAX_DEPTH )) && return

    # Fetch directory
    local BODY_FILE LINK_FILE HTTP_CODE BODY
    BODY_FILE=$(mktemp "$TEMP_DIR/dir_body.XXXXXX")
    HTTP_CODE=$(curl "${CURL_DIR_ARGS[@]}" -o "$BODY_FILE" -w '%{http_code}' "$URL" 2>/dev/null)
    BODY=$(cat "$BODY_FILE" 2>/dev/null)
    rm -f "$BODY_FILE"

    # Check for directory listing
    if ! echo "$BODY" | grep -qiE "$DIRLIST_PATTERN"; then
        return
    fi

    ((DIRLIST_FOUND++))
    local INDENT=$(printf '%*s' $((DEPTH * 2)) '')
    echo -e "${GREEN}    ${INDENT}✓ Listing found: ${URL}${NC}"

    # Parse links from listing into temp file
    LINK_FILE=$(mktemp "$TEMP_DIR/dir_links.XXXXXX")
    echo "$BODY" | grep -oiE 'href="[^"]*"' | \
        sed 's/href="//i; s/"$//' | \
        grep -v '^\.\.' | \
        grep -v '^\?' | \
        grep -v '^#' | \
    ...
    while IFS= read -r link; do
        ...
        local FULL
        case "$link" in
            http://*|https://*) FULL="$link" ;;
            /*)                 FULL="${TARGET_ORIGIN}${link}" ;;
            *)                  FULL="${URL%/}/${link}" ;;
        esac

        echo "$FULL" >> "$TEMP_DIR/dirlist_urls.txt"

        # Recurse into subdirectories (ends with /)
        if [[ "$link" == */ ]]; then
            crawl_dirlist "$FULL" $((DEPTH + 1))
        fi
    done < "$LINK_FILE"
...
while IFS= read -r dir_url; do
    crawl_dirlist "$dir_url" 0
done < "$TEMP_DIR/dir_candidates.txt"

# ── Produce output files ─────────────────────────────────────────────────
if [ -s "$TEMP_DIR/dirlist_urls.txt" ]; then
    # Tool file: unique discovered URLs, filtered (no static junk)
    sort -u "$TEMP_DIR/dirlist_urls.txt" | \
        grep -viE "$STATIC_FILTER" | \
        grep -viE "$JS_FILTER" \
        > "$OUTPUT_DIR/dirlist/endpoints.txt"

    # Detail file: probe with httpx for status + size
    if [ -s "$OUTPUT_DIR/dirlist/endpoints.txt" ]; then
        cat "$OUTPUT_DIR/dirlist/endpoints.txt" | \
            httpx "${HTTPX_BASE_ARGS[@]}" 2>/dev/null | \
            jq -r '"\(.status_code // "-") \(.content_length // 0)B \(.url // "-")"' | \
            sort -u > "$OUTPUT_DIR/dirlist/endpoints_detail.txt"
    fi
else
    touch "$OUTPUT_DIR/dirlist/endpoints.txt" "$OUTPUT_DIR/dirlist/endpoints_detail.txt"
fi

touch "$OUTPUT_DIR/dirlist/endpoints.txt" "$OUTPUT_DIR/dirlist/endpoints_detail.txt"

COUNT_DIRLIST=$(wc -l < "$OUTPUT_DIR/dirlist/endpoints.txt" 2>/dev/null || echo 0)
echo -e "${GREEN}    ✓ ${DIRLIST_FOUND} listings found, ${COUNT_DIRLIST} new endpoints${NC}"
...
cat "$OUTPUT_DIR"/katana/endpoints.txt \
    "$OUTPUT_DIR"/gau/endpoints.txt \
    "$OUTPUT_DIR"/ffuf/endpoints.txt \
    "$OUTPUT_DIR"/dirlist/endpoints.txt 2>/dev/null | \
...
# GAU + ffuf + dirlist don't have METHOD → prefix with GET
{
    ...
    # dirlist: "STATUS SIZEB URL" → "GET STATUS SIZEB URL"
    awk '{print "GET " $0}' "$OUTPUT_DIR/dirlist/endpoints_detail.txt" 2>/dev/null
...
echo -e "    ${OUTPUT_DIR}/dirlist/endpoints.txt / endpoints_detail.txt"
echo ""
